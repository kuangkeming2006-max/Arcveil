...
// Rewrites the exact PlayerControllerMP::attackEntity entry so the original
// call remains the single dispatch source while native arbitration may replace
// its target. A null decision returns before vanilla emits an attack.
inline jbyteArray weaveAttackMethod(JNIEnv* env,
    std::span<const unsigned char> bytes,const char* methodName,
    const char* methodDescriptor,const char* targetInternalName) noexcept {
...
#define ATTACK_DO(expression) expression; if(env->ExceptionCheck()) return nullptr
    ...
    ATTACK_GET(bridgeName,env->NewStringUTF("mcoverlay/NativeAttackBridge_v40"));
    ATTACK_GET(bridgeMethod,env->NewStringUTF("arbitrateAttack"));
    ...
    for(jint index=0;index<count;++index) {
        ...
        ATTACK_GET(original,env->NewObject(varInsnClass,varInsnCtor,25,2));
        ATTACK_DO(env->CallVoidMethod(entry,add,original));
        ...
        ATTACK_GET(duplicate,env->NewObject(insnClass,insnCtor,89));
        ATTACK_DO(env->CallVoidMethod(entry,add,duplicate));
        ATTACK_GET(continueLabel,env->NewObject(labelClass,labelCtor));
        ATTACK_GET(nonNull,env->NewObject(jumpClass,jumpCtor,199,continueLabel));
        ATTACK_DO(env->CallVoidMethod(entry,add,nonNull));
        ATTACK_GET(pop,env->NewObject(insnClass,insnCtor,87));
        ATTACK_DO(env->CallVoidMethod(entry,add,pop));
        ...
        ATTACK_GET(stack,env->NewObjectArray(1,objectClass,nullptr));
        ATTACK_GET(frameObject,env->NewStringUTF("java/lang/Object"));
        ATTACK_DO(env->SetObjectArrayElement(stack,0,frameObject));
        ATTACK_GET(frame,env->NewObject(frameClass,frameCtor,4,0,nullptr,1,stack));
        ATTACK_DO(env->CallVoidMethod(entry,add,frame));
        ATTACK_GET(cast,env->NewObject(typeInsnClass,typeInsnCtor,192,targetType));
        ATTACK_DO(env->CallVoidMethod(entry,add,cast));
        ATTACK_GET(store,env->NewObject(varInsnClass,varInsnCtor,58,2));
        ATTACK_DO(env->CallVoidMethod(entry,add,store));
