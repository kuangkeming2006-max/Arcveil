...
#include "AttackMethodWeaver.h"
#include "NativeAttackBridgeBytes.h"
...
// Owns the stable lower PlayerControllerMP::attackEntity boundary. Native code
// selects one target, then the original method performs exactly one dispatch.
class LiveAttackTransform final {
public:
    using Callback=jobject(*)(void*,JNIEnv*,jobject) noexcept;
    ~LiveAttackTransform(){stop();}
    ...
    bool install(JavaVM* vm,jmethodID method,void* owner,Callback callback) noexcept {
        ...
        m_env->Deallocate(reinterpret_cast<unsigned char*>(rawName));
        m_env->Deallocate(reinterpret_cast<unsigned char*>(rawDescriptor));
        const auto firstEnd=m_descriptor.find(';');
        const auto secondStart=m_descriptor.find('L',firstEnd==std::string::npos?0:firstEnd+1);
        const auto secondEnd=m_descriptor.find(';',secondStart);
        if(secondStart==std::string::npos||secondEnd==std::string::npos){stop();return false;}
        try{m_targetInternal=m_descriptor.substr(secondStart+1,secondEnd-secondStart-1);}
        ...
        jstring bridgeName=jni->NewStringUTF("mcoverlay.NativeAttackBridge_v40");
        ...
        if(!bridge)bridge=jni->DefineClass("mcoverlay/NativeAttackBridge_v40",loader,
            reinterpret_cast<const jbyte*>(nativeAttackBridgeBytes),
            sizeof(nativeAttackBridgeBytes));
        ...
        JNINativeMethod native{const_cast<char*>("arbitrateAttack"),
...
private:
    static jobject JNICALL dispatch(JNIEnv* env,jclass,jobject original) noexcept {
        jobject result=original;AcquireSRWLockShared(&s_gate);
        auto* self=s_instance;
        if(self&&self->m_callback&&self->m_enabled.load(std::memory_order_acquire))
            result=self->m_callback(self->m_owner,env,original);
    ...
    static void JNICALL transform(jvmtiEnv* env,JNIEnv* jni,jclass target,
        ...
        jbyteArray output=weaveAttackMethod(jni,
            {bytes,static_cast<std::size_t>(length)},self->m_name.c_str(),
            self->m_descriptor.c_str(),self->m_targetInternal.c_str());
    ...
    inline static LiveAttackTransform* s_instance=nullptr;
