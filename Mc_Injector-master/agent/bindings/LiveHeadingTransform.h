...
#include "NativeSprintBridgeBytes.h"
...
// PRE runs before EntityLivingBase computes getAIMoveSpeed/ground friction.
// POST is observational only; it never restores sprint.
class LiveHeadingTransform final {
public:
    using Callback=void(*)(void*,JNIEnv*,jobject) noexcept;

    ~LiveHeadingTransform() { stop(); }
    ...
    bool install(JavaVM* vm,jmethodID method,void* owner,
                 Callback pre,Callback post) noexcept {
        if(m_ready) return true;
        if(!vm||!method||!pre||!post) return false;
        JNIEnv* jni=nullptr;
        ...
        if((m_error=m_env->GetMethodName(method,&name,&descriptor,nullptr))!=JVMTI_ERROR_NONE)
        {stop();return false;}
        ...
        jstring bridgeName=jni->NewStringUTF("mcoverlay.NativeSprintBridge_v53");
        ...
        if(!bridge) bridge=jni->DefineClass("mcoverlay/NativeSprintBridge_v53",loader,
            reinterpret_cast<const jbyte*>(nativeSprintBridgeBytes),
            sizeof(nativeSprintBridgeBytes));
        ...
        if(!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS|
               ...
               reinterpret_cast<LPCWSTR>(&dispatchPre),&module)) {
            jni->DeleteLocalRef(bridge);stop();return false;
        }
        JNINativeMethod natives[]{
            {const_cast<char*>("headingPre"),
             const_cast<char*>("(Ljava/lang/Object;)V"),
             reinterpret_cast<void*>(&dispatchPre)},
            {const_cast<char*>("headingPost"),
             const_cast<char*>("(Ljava/lang/Object;)V"),
             reinterpret_cast<void*>(&dispatchPost)}};
        ...
        if(!occupied) {s_instance=this;m_owner=owner;m_pre=pre;m_post=post;}
    ...
    void stop() noexcept {
        ...
        if(m_vm&&m_vm->GetEnv(reinterpret_cast<void**>(&jni),JNI_VERSION_1_6)==
               JNI_OK&&m_target) jni->DeleteGlobalRef(m_target);
        m_env->DisposeEnvironment();
        m_env=nullptr;m_target=nullptr;m_ready=false;m_transformed=false;
    }
private:
    static void JNICALL dispatchPre(JNIEnv* jni,jclass,jobject subject) noexcept {
        AcquireSRWLockShared(&s_gate);
        LiveHeadingTransform* self=s_instance;
        if(self&&self->m_pre&&self->m_enabled.load(std::memory_order_acquire))
            self->m_pre(self->m_owner,jni,subject);
        ReleaseSRWLockShared(&s_gate);
    }
    static void JNICALL dispatchPost(JNIEnv* jni,jclass,jobject subject) noexcept {
        AcquireSRWLockShared(&s_gate);
        LiveHeadingTransform* self=s_instance;
        if(self&&self->m_post) self->m_post(self->m_owner,jni,subject);
        ReleaseSRWLockShared(&s_gate);
    }
    static void JNICALL transform(jvmtiEnv* env,JNIEnv* jni,jclass target,
        ...
        jbyteArray output=weaveJumpMethod(jni,
            ...
            self->m_descriptor.c_str(),"mcoverlay/NativeSprintBridge_v53",
            "headingPre","headingPost");
    ...
    inline static LiveHeadingTransform* s_instance=nullptr;
    JavaVM* m_vm=nullptr;
